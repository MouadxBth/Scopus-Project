...
public class ScopusEntry {
    ...
    public List<ScopusLink> links() {
        if (LINKS.isEmpty() && link != null && !link.isEmpty()) {
            link.asList()
                    .stream()
                    .map(element -> ScopusJavaProjectApplication.GSON.fromJson(element, ScopusLink.class))
                    .forEach(LINKS::add);
        }
        return LINKS;
    }
    ...
    public List<ScopusAffiliate> affiliates() {
        if (AFFILIATES.isEmpty() && affiliation != null && !affiliation.isEmpty())
                affiliation.asList()
                .stream()
                .map(element -> ScopusJavaProjectApplication.GSON.fromJson(element, ScopusAffiliate.class))
